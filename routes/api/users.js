                            ...
                            token.save()
                                .then(token => {
                                    ...
                                    const message = {
                                        to: user.email,
                                        from: receiverEmail,
                                        subject: "FitSpire Account Verification",
                                        text: `Please verify your FitSpire account here: ${host}/verify/${token.token}
                                        This link will expire in 72 hours. If you did not create an account with FitSpire, please ignore this email.`,
                                        html: `<p>Please verify your FitSpire account here: 
                                        <a href="${host}/verify/${token.token}">${host}/verify/${token.token}</a> This link will expire in 72 hours.</p>
                                        <p>If you did not create an account with FitSpire, please ignore this email.</p>`
                                    };
                                    sgMail.send(message)
        ...
        token.save()
            .then(token => {
                ...
                const message = {
                    to: user.email,
                    from: receiverEmail,
                    subject: "FitSpire Password Reset",
